...
look_descriptions = {
    ...
    "door": "The door look sturdy. It's locked",
            ...
            if blob['entity'] == 'object':
                ...
                if item not in able_to_pick_up:
                    ...
                    if item_in_inventory:
                        ...
                        dispatcher.utter_message(text=f"You've picked up the {item} in your inventory.")
...
combinations = {
    ('key', 'door'): 'Hey the key fits! Amazing! The door is nox open.',
...
class ActionUse(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if len(entities) == 1:
            dispatcher.utter_message(text="I think you want to combine items but something is unclear.")
            dispatcher.utter_message(text=f"I could only make out that you wanted to us {entities[0]}")
            dispatcher.utter_message(text="Could you retry and make sure you spelled the items correctly?")
            return []
        if len(entities) > 2:
            dispatcher.utter_message(text="I think you want to combine items but something is unclear.")
            dispatcher.utter_message(text=f"I could only make out that you wanted to combine {' and'.join(entities)}.")
            dispatcher.utter_message(text=f"You can only combine two items at a time.")
            return []
